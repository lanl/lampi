...
void FixedSharedMemPool::
init(ssize_t initialAllocation, ssize_t minAllocationSize, int numberPools,
     int nArrayElementsToAdd, bool applyMemAffinity)
{
    ...
    if( !memSegments_m) {
        ulm_exit((-1, "FixedSharedMemPool::init Unable to allocate memory for "
	...
	if( !memSegments_m[pool]) {
            ulm_exit((-1, "FixedSharedMemPool::init Unable to allocate memory "
	...
	for( int pool=0 ; pool < nPools_m ; pool++ ) {
	    ...
	    // allocate memory
	    void *tmpPtr=
		ZeroAlloc(initialAllocation, MMAP_SHARED_PROT,
                          MMAP_SHARED_FLAGS);
	    if( !tmpPtr ) {
                ulm_exit((-1, "FixedSharedMemPool::init Unable to allocate "
                          "memory pool , requested %ld, errno %d \n",
                          initialAllocation, errno));
	    }
	    // apply memory affinity
	    if( applyMemoryAffinity ) {
		int returnValue = setMemoryPolicy(tmpPtr, initialAllocation,
                                                  pool);
		if( returnValue != ULM_SUCCESS) {
                    ulm_exit((-1, "FixedSharedMemPool::init error setting "
                              "memory policy \n"));
		}
	    }
...
void *FixedSharedMemPool::getMemorySegment(ssize_t length, ssize_t alignment,
					   int poolIndex)
{
    ...
    for( int segment=0 ; segment< nMemorySegments_m[poolIndex] ; segment++ ) {

	char *ptr=(char *)memSegments_m[poolIndex][segment].currentPtr_m;
	...
	if( ( ((size_t)ptr) & mask) == ((size_t)ptr) ) {
	    ...
	    ssize_t avail=memSegments_m[poolIndex][segment].memAvailable_m
	...
	if( memSegments_m[poolIndex][segment].memAvailable_m >= lengthToAllocate ){
	...
	if( nMemorySegments_m[poolIndex] == nMemorySegmentsInArray_m[poolIndex] ) {
	    // create a temp version of memSegments_m[]
	    MemorySegment_t *tmpMemSeg = (MemorySegment_t *)
		ulm_malloc(sizeof(MemorySegment_t)*
                           (nMemorySegments_m[poolIndex]+nArrayElementsToAdd_m) );
	    if( !tmpMemSeg) {
	...
	// apply memory affinity
	if( applyMemoryAffinity ) {
	    int returnValue=setMemoryPolicy(tmpPtr, lenToAllocate, poolIndex);
	    if( returnValue != ULM_SUCCESS) {
                ulm_exit((-1, "FixedSharedMemPool::getMemorySegment error"
	...
	memSegments_m[poolIndex][segmentIndex].segmentLength_m=lenToAllocate;
	memSegments_m[poolIndex][segmentIndex].memAvailable_m=lenToAllocate;

	//
	nMemorySegments_m[poolIndex]++;

	// set pointer and length
	char *ptr=(char *)memSegments_m[poolIndex][segmentIndex].currentPtr_m;
	// check to see if pointer is aligned correctly
	if( ( ((size_t)ptr) & mask) == ((size_t)ptr) ) {
	    returnPtr=ptr;
	    lengthToAllocate=length;
	    ...
	    returnPtr=ptr;
	    lengthToAllocate=length+
		(ptr-(char *)memSegments_m[poolIndex][segmentIndex].currentPtr_m);
	}
    ...
    // set memAvailable_m
    memSegments_m[poolIndex][segmentIndex].memAvailable_m-=lengthToAllocate;
