...
/* this version of bcopy_csum() looks a little too long, but it handles cumulative
 * checksumming for arbitrary lengths and address alignments as best as it can;
 * the contents of lastPartialLong and lastPartialLength are updated to reflected
 * the last partial word's value and length (in bytes) -- this should allow
 * proper handling of checksumming contiguous or noncontiguous buffers via multiple
 * calls of bcopy_csum() - Mitch
 */
...
/* this version is appropriate for checksumming a buffer via one call to bcopy_csum() */
...
/* this version is appropriate for checksumming a buffer via one call to bcopy_uicsum() */
...
/* csum() generates a bcopy_csum() - compatible checksum that can
