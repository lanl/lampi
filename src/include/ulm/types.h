 ...
 */
typedef void *ULMRequestHandle_t;

/*
 * struct containing status information for a message
 */
typedef struct ULMStatus_t ULMStatus_t;
struct ULMStatus_t {
    int tag;			/*!< user defined tag */
    union {
	int source;		/*!< for a recv the source process */
	int destination;	/*!  for a send the destination process */
    } proc;
    int state;			/*!  status of request */
    int error;			/*!  error sttus */
    ssize_t matchedSize;	/*!  for send, same as posted send */
    int persistent;
    /* for receive, the size actually received
       sizes are in bytes */
};
