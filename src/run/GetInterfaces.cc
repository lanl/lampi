...
{
    RunParameters.NInterfaces = 0;
    RunParameters.InterfaceList = 0;
}
...
void GetInterfaceList(const char *InfoStream)
{
    ...
    int OptionIndex =
        MatchOption("InterfaceList", ULMInputOptions, SizeOfInputOptionsDB);
    if (OptionIndex < 0) {
        ulm_err(("Error: Option InterfaceList not found\n"));
        Abort();
    }

    ParseString InterfaceList(ULMInputOptions[OptionIndex].InputData,
                              NSeparators, SeparatorList);

    RunParameters.NInterfaces = InterfaceList.GetNSubStrings();
    if(RunParameters.NInterfaces == 0) {
        ulm_err(("usage: mpirun [-i <interface-name-list>]\n"));
        Abort();
    }

    size = sizeof(InterfaceName_t) * RunParameters.NInterfaces;
    RunParameters.InterfaceList = (InterfaceName_t*)ulm_malloc(size);
    if(RunParameters.InterfaceList == 0) {
        ulm_err(("GetInterfaceList: ulm_malloc(%d) failed.\n", size));
        Abort();
    }
    memset(RunParameters.InterfaceList, 0, size);

    int index=0;
    for (ParseString::iterator i = InterfaceList.begin(); i != InterfaceList.end(); i++) {
        HostName_t if_addr;
        if(ulm_ifnametoaddr(*i, if_addr, sizeof(if_addr)) != ULM_SUCCESS) {
            ulm_err(("Invalid interface: %s\n", *i));
            Abort();
        }
        strcpy(RunParameters.InterfaceList[index++], *i);
    }
}
...
void GetInterfaceCount(const char *InfoStream)
{
    ...
    int OptionIndex =
        MatchOption("InterfaceCount", ULMInputOptions, SizeOfInputOptionsDB);
    if (OptionIndex < 0) {
    ...
    RunParameters.NInterfaces = ifCount;
    RunParameters.InterfaceList = (InterfaceName_t*)ulm_malloc(ifCount * sizeof(InterfaceName_t));
    int index=0;
    int ifIndex = ulm_ifbegin();
    while(ifIndex > 0 && index < ifCount) {
        ulm_ifindextoname(ifIndex, RunParameters.InterfaceList[index++], sizeof(InterfaceName_t));
