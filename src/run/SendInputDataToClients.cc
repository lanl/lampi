		...
		adminMessage *server)
{
	...
	if (!server->send(host,dev_type_params::START_SHARED_MEM_INPUT,
				&errorCode)) {
		ulm_err(("unable to send START_SHARED_MEM_INPUT message (error %d)\n", errorCode));
    		Abort();
	}
#endif
		...
		adminMessage *server)
{
    ...
    if ( !server->broadcast(dev_type_params::START_QUADRICS_INPUT,
		    &errorCode) )
	{
		ulm_err(("unable to broadcast quadrics input message (error %d)\n", errorCode));
    		Abort();
	}
		...
		adminMessage *server)
{
    ...
    if ( !server->broadcast(dev_type_params::START_GM_INPUT,
		    &errorCode) )
	{
		ulm_err(("unable to broadcast GM input message (error %d)\n", errorCode));
    		Abort();
	}
#endif
