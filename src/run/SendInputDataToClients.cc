		...
		adminMessage *server)
{
    ...
    if (!server->
        pack(&cid, (adminMessage::packType) sizeof(unsigned int), 1))
        DataError("ChannelID");
    ...
    if (!server->
        pack(&(RunParameters->OutputPrefix),
             (adminMessage::packType) sizeof(int), 1))
        DataError("CheckArgs");
    ...
    if (RunParameters->nCpusPerNode != 0) {
        ...
        if (!server->
            pack(&RunParameters->nCpusPerNode,
                 (adminMessage::packType) sizeof(int), 1))
    ...
    for (label = 0; label < (unsigned int) nhosts; label++) {

        host = server->clientDaemon2Rank(label);
        ...
        if (RunParameters->irecvResources_m.minPagesPerContext_m != NULL) {
            ...
            if (!server->
                pack(&
                     ((RunParameters->irecvResources_m.
    ...
    if (!server->scatterv(0, adminMessage::RUNPARAMS, &errorCode)) {
        ulm_err(("Error: Can't send RUNPARAMS message (error %d)\n",
                 errorCode));
        Abort();
    }
                                ...
                                adminMessage * server)
{
    ...
    for (label = 0; label < (unsigned int) RunParameters->NHosts; label++) {
        ...
        if (RunParameters->smDescPoolBytesPerProc != 0) {
            ...
            if (!server->
                pack(((RunParameters->smDescPoolBytesPerProc) + host),
                     (adminMessage::packType) sizeof(ssize_t), 1))
                         ...
                         adminMessage * server)
{
    ...
    if (RunParameters->Networks.GMSetup.NGMHosts == 0) {
        return returnValue;
    } else if (RunParameters->Networks.GMSetup.NGMHosts !=
