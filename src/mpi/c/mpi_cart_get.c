		  ...
		  int *coords)
{
    ...
    if (maxdims < 1 || dims == NULL || periods == NULL || coords == NULL) {
	rc = MPI_ERR_ARG;
	goto ERRHANDLER;
    }
    ...
    if (topology->cart.ndims > maxdims) {
	rc = MPI_ERR_DIMS;
	goto ERRHANDLER;
    }
    for (i = 0; i < topology->cart.ndims; i++) {
	dims[i] = topology->cart.dims[i];
	periods[i] = topology->cart.periods[i];
	coords[i] = topology->cart.position[i];
    }
