		   ...
		   int *edges)
{
    ...
    if (maxindex < 1 || maxedges < 1 || index == NULL || edges == NULL) {
	rc = MPI_ERR_ARG;
	goto ERRHANDLER;
    }
	...
	if (topology->graph.nnodes > maxindex) {
	    rc = MPI_ERR_DIMS;
	    goto ERRHANDLER;
	}
	for (i = 0; i < topology->graph.nnodes; i++) {
	    index[i] = topology->graph.index[i];
	}
	if (topology->graph.nedges > maxedges) {
	    rc = MPI_ERR_DIMS;
	    goto ERRHANDLER;
	}
	for (i = 0; i < topology->graph.nedges; i++) {
	    edges[i] = topology->graph.edges[i];
	}
    } else {
	rc = MPI_ERR_TOPOLOGY;
    }
