...
int PMPI_Ssend_init(void *buf, int count, MPI_Datatype type, int dest,
		    int tag, MPI_Comm comm, MPI_Request *request)
{
	...
	*request = _mpi.proc_null_request_persistent;
    ...
    /*
     * the synchronous send is implemented as a ready send to improve
     * the performance of the non-blocking send
     */
    rc = ulm_isend_init(buf, count, datatype, dest,
			tag, comm, &req, ULM_SEND_SYNCHRONOUS, persistent);
