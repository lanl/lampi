...
private:
    ...
    bool getRecvBytes(int bytes, int timeout) {
        ...
        while (bytesReceived < bytes) {
            int bytesThisRead = read(lastRecvSocket_m, (void *)(recvBuffer_m + recvBufferBytes_m), bytes - bytesReceived);
            if (bytesThisRead <= 0) {
                returnValue = false;
                break;
            }
...
public:
	...
	*/
    ...
    /* this primitive implementation of allgather handles only contiougs data.
	...
	/*
		This version of scatterv assumes that the scatter is being initiated
		by the mpirun process.
	*/
    ...
    /* this routine sets up this collective data for use by this object's
     *   collective routines */
    ...
    recvResult receive(int rank, int *tag, int *errorCode, int timeout = -1) {
        ...
        *tag = -1;
        ...
        if ((s = select(sockfd+1, &fds, (fd_set *)NULL, (fd_set *)NULL,
            ...
            if ( (sizeRecv=ulm_readv(sockfd, &iovecs, 1)) != sizeof(int)) {
                *errorCode = errno;
                returnValue = ERROR;
                return returnValue;
            }
