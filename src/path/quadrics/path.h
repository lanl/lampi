...
public:
    ...
    void unbind(BaseSendDesc_t * message, int *globalDestProcessIDArray,
        ...
        {
            ulm_exit((-1,
                      "quadricsPath::unbind - called, not implemented yet...\n"));
        }
    ...
    bool init(BaseSendDesc_t * message)
        {
            ...
            npkts =
                (message->posted_m.length_m + quadricsBufSizes[lbufType] -
                 1) / quadricsBufSizes[lbufType];
            if (npkts == 0)
                npkts = 1;
    ...
    bool sendDone(BaseSendDesc_t * message, double timeNow, int *errorCode)
        {
            ...
            if (!quadricsDoAck) {
                ...
                if ((message->sendType != ULM_SEND_SYNCHRONOUS)
                    ...
                    for (sfd =
                             (quadricsSendFragDesc *) message->FragsToAck.begin();
                         sfd !=
                             (quadricsSendFragDesc *) message->FragsToAck.end();
                        ...
                        if (sfd->done(timeNow, errorCode)) {
                            ...
                            if (usethreads()) {
                                quadricsSendFragDescs.returnElement(afd,
                                                                    afd->rail);
                            } else {
    ...
    bool retransmitP(BaseSendDesc_t * message)
        {
            if (!quadricsDoAck || (RETRANS_TIME == -1)
    ...
    bool getCtxRailAndDest(BaseSendDesc_t * message,
                           int globalDestProcessID, ELAN3_CTX ** ctx,
                           int *rail, void **dest, int destBufType,
                           bool needDest, int *errorCode)
        {
            int i, r, lr = quadricsLastRail;
            ...
            for (i = 1; i <= quadricsNRails; i++) {
                ...
                if (quadricsQueue[r].railOK) {
                    railAvail = true;
                    if (needDest) {
                        quadricsPeerMemory[r]->addrCounts(globalDestProcessID,
                                                          bufCounts);
                        if (bufCounts[destBufType] == 0)
                            continue;
                        *dest =
                            quadricsPeerMemory[r]->pop(globalDestProcessID,
                                                       destBufType);
                        if (!*dest)
                            continue;
                        *rail = r;
    ...
    bool sendCtlMsgs(int rail, double timeNow, int startIndex,
                     int endIndex, int *errorCode, bool skipCheck = false);

    // all rails version
    bool sendCtlMsgs(double timeNow, int startIndex, int endIndex,
                     int *errorCode);

    bool cleanCtlMsgs(int rail, double timeNow, int startIndex,
                      int endIndex, int *errorCode, bool skipCheck =
                      false);

    // all rails version
    bool cleanCtlMsgs(double timeNow, int startIndex, int endIndex,
                      int *errorCode);

    bool sendMemoryRequest(BaseSendDesc_t * message, int gldestProc,
                           size_t offset, size_t memNeeded,
                           int *errorCode);
