...
bool quadricsRecvFragDesc::AckData(double timeNow)
{
    ...
    unsigned int glSourceProcess =
        pg->remoteGroup->mapGroupProcIDToGlobalProcID[srcProcID_m];

    if (isDuplicate_m) {
        p->ackStatus = ACKSTATUS_DATAGOOD;
    } else {
        p->ackStatus =
            (DataOK) ? ACKSTATUS_DATAGOOD : ACKSTATUS_DATACORRUPT;
    }
#else
    ...
    if ((msgType_m == MSGTYPE_PT2PT) || (msgType_m == MSGTYPE_PT2PT_SYNC)) {
        ...
        bool send_specific_ack =
            reliabilityInfo->deliveredDataSeqs[glSourceProcess].
        ...
        if (!(isDuplicate_m)) {
            if (DataOK) {
                if (!recorded) {
                    reliabilityInfo->dataSeqsLock[glSourceProcess].
                        unlock();
                    ulm_exit((-1,
                              "quadricsRecvFragDesc::AckData(pt2pt) unable "
                              "to record deliv'd sequence number\n"));
                }
            } else {
                if (!
                    (reliabilityInfo->receivedDataSeqs[glSourceProcess].
                     erase(seq_m))) {
                    reliabilityInfo->dataSeqsLock[glSourceProcess].
                        unlock();
                    ulm_exit((-1,
            ...
            if (!
                ...
                ulm_exit((-1,
                          "quadricsRecvFragDesc::AckData(pt2pt) unable to erase duplicate deliv'd sequence number\n"));
            }
        }
        ...
        ulm_exit((-1,
                  "quadricsRecvFragDesc::AckData() unknown communication "
                  "type %d\n", msgType_m));
    }
#else
    p->receivedFragSeq = 0;
    ...
    path->sendCtlMsgs(rail, timeNow, MESSAGE_DATA_ACK, MESSAGE_DATA_ACK,
                      &errorCode);
    path->cleanCtlMsgs(rail, timeNow, MESSAGE_DATA_ACK, MESSAGE_DATA_ACK,
                       &errorCode);
    return true;
}
...
bool quadricsRecvFragDesc::isDuplicateCollectiveFrag()
{
    ...
    if (seq_m != 0) {
        ...
        if (reliabilityInfo->coll_receivedDataSeqs[source_box].
            recordIfNotRecorded(seq_m, &recorded)) {
            // this is a duplicate from a retransmission...maybe a previous ACK was lost?
            // unlock so we can lock while building the ACK...
            if (reliabilityInfo->coll_deliveredDataSeqs[source_box].
                isRecorded(seq_m)) {
                // send another ACK for this specific frag...
                isDuplicate_m = true;
                sendthisack = true;
            } else if (reliabilityInfo->coll_receivedDataSeqs[source_box].
                       largestInOrder() >= seq_m) {
...
bool quadricsRecvFragDesc::
checkForDuplicateAndNonSpecificAck(quadricsSendFragDesc * sfd)
{
    ...
    if ((msgType_m == MSGTYPE_PT2PT) || (msgType_m == MSGTYPE_PT2PT_SYNC)) {
        ...
        ulm_err(("quadricsRecvFragDesc::check...Ack received ack of unknown " "message type %d\n", msgType_m));
        return true;
    }
...
void quadricsRecvFragDesc::handlePt2PtMessageAck(double timeNow,
                                                 BaseSendDesc_t * bsd,
                                                 ...
                                                 sfd)
{
    ...
    if (p->ackStatus == ACKSTATUS_DATAGOOD) {
        ...
        if (whichQueue == QUADRICSFRAGSTOACK) {
            ...
            ulm_exit((-1,
                      "quadricsRecvFragDesc::processAck: Frag "
                      "on %d queue\n", whichQueue));
        }
        ...
        if (sfd->done(timeNow, &errorCode)) {
            ...
            if (usethreads()) {
                ...
                list->AppendNoLock((Links_t *) sfd);
                quadricsQueue[rail].ctlFlagsLock.lock();
                quadricsQueue[rail].ctlMsgsToAckFlag |=
                    (1 << MESSAGE_DATA);
                quadricsQueue[rail].ctlFlagsLock.unlock();
                list->Lock.unlock();
            } else {
...
void quadricsRecvFragDesc::msgData(double timeNow)
{
    ...
    addr_m =
        (length_m <=
         CTLHDR_DATABYTES) ? (void *) p->
        data : (void *) elan3_elan2main(ctx, (E3_Addr) p->dataElanAddr);
    if (usethreads())
        quadricsState.quadricsLock.unlock();
    ...
    if (dstProcID_m != myproc()) {
        ulm_exit((-1,
                  "Quadrics data frag on rail %d from process %d misdirected to process "
                  "%d, intended destination %d. frag_seq = %lld\n", rail,
                  srcProcID_m, myproc(), dstProcID_m, seq_m));
    }
    ...
    if ((msgType_m == MSGTYPE_PT2PT) || (msgType_m == MSGTYPE_PT2PT_SYNC)) {
        ...
        dstProcID_m =
            communicators[ctx_m]->localGroup->
            mapGlobalProcIDToGroupProcID[dstProcID_m];
        srcProcID_m =
            communicators[ctx_m]->remoteGroup->
            mapGlobalProcIDToGroupProcID[srcProcID_m];

        // what do we do if the return value is not ULM_SUCCESS?
        communicators[ctx_m]->
            handleReceivedFrag((BaseRecvFragDesc_t *) this, timeNow);

    } else {
        ulm_exit((-1,
                  "Quadrics data envelope with invalid message "
                  "type %d\n", msgType_m));
    }
    return;
}
...
void quadricsRecvFragDesc::msgDataAck(double timeNow)
{
    quadricsDataAck_t *p = &(envelope.msgDataAck);
    quadricsSendFragDesc *sfd =
        (quadricsSendFragDesc *) p->sendFragDescPtr.ptr;
    volatile BaseSendDesc_t *bsd =
        (volatile BaseSendDesc_t *) sfd->parentSendDesc;
...
void quadricsRecvFragDesc::memRel()
{
    ...
    if ((p->destID != myproc())
        || quadricsMemRlsSeqList[p->senderID].isRecorded(p->releaseSeq)) {
        ReturnDescToPool(getMemPoolIndex());
        return;
    }
    ...
    default:
        ulm_exit((-1,
                  "quadricsRecvFragDesc::memRel: bad memory " "type %d!\n",
                  p->memType));
        break;
    }
    ...
    if (!quadricsMemRlsSeqList[p->senderID].record(p->releaseSeq)) {
        ulm_exit((-1,
                  "quadricsRecvFragDesc::memRel: unable to record "
...
void quadricsRecvFragDesc::memReq(double timeNow)
{
    ...
    case MSGTYPE_PT2PT_SYNC:
        if (neededBytes <= quadricsBufSizes[PRIVATE_SMALL_BUFFERS]) {
            ...
            naddrsNeeded =
                (neededBytes + quadricsBufSizes[bufType] -
                 1) / quadricsBufSizes[bufType];
        }
        break;
    }
    ...
    for (int i = 0; i < naddrsNeeded; i++) {
        if ((mtype == MSGTYPE_PT2PT) || (mtype == MSGTYPE_PT2PT_SYNC)) {
            ...
            if (usethreads())
                quadricsState.quadricsLock.lock();
            addrs[i] =
                elan3_allocMain(ctx, E3_BLK_ALIGN,
            ...
            addrs[i] =
                allocPayloadBuffer(dev, quadricsBufSizes[bufType],
    ...
    // use incoming ctx and rail!
    sfd->init(0, ctx, MEMORY_REQUEST_ACK, rail, p->senderID, -1, 0, 0, 0,
              0, 0, false, hdr);
    ...
    path->sendCtlMsgs(rail, timeNow, MEMORY_REQUEST_ACK,
                      MEMORY_REQUEST_ACK, &errorCode);
    path->cleanCtlMsgs(rail, timeNow, MEMORY_REQUEST_ACK,
                       MEMORY_REQUEST_ACK, &errorCode);
...
void quadricsRecvFragDesc::memReqAck()
{
    ...
    case MEMREQ_SUCCESS:
        ...
        // tracking structure
        quadricsPeerMemory[rail]->push((int) p->senderID, (int) p->memType,
                                       addrs, (int) p->memBufCount);
        break;
    case MEMREQ_FAILURE_FATAL:
        ulm_exit((-1,
                  "quadricsRecvFragDesc::memReqAck() fatal "
                  "memory request error from global" " process %d\n",
                  p->senderID));
        break;
