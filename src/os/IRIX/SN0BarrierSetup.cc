...
void fetchAndAddSMPHWBarrier(volatile void *barrierData)
{
    ...
    barData->currentCounter = FETCHOP_LOAD_OP(barData->fetchOpVar, FETCHOP_INCREMENT);
    while (barData->currentCounter < barData->releaseCnt) {
        barData->currentCounter = FETCHOP_LOAD_OP(barData->fetchOpVar, FETCHOP_LOAD);
        count++;
        if (count == 10000) {
            ulm_make_progress();
            count = 0;
                                       ...
                                       int *gotNetworkResources)
{
    ...
    *callGenericNetworkSetup = 1;
    ...
    for (int pl = 0; pl < ULMai->hwBarrier.nPools; pl++) {
        for (int ele = 0; ele < ULMai->hwBarrier.nElementsInPool[pl]; ele++) {
            if (ULMai->hwBarrier.pool[pl][ele].inUse &&
                (ULMai->hwBarrier.pool[pl][ele].contextID == contextID) &&
                (ULMai->hwBarrier.pool[pl][ele].commRoot == localGroup->mapGroupProcIDToGlobalProcID[0])) {
    ...
    if (firstInstanceOfContext) {
        ...
        if (ULMai->hwBarrier.nPools > 0) {
            ...
            int poolCnt = 0;
            while ((!foundFreeElement) && (poolCnt < ULMai->hwBarrier.nPools)) {
                ...
                for (int ele = 0; ele < ULMai->hwBarrier.nElementsInPool[poolIndex]; ele++) {
                    ...
                    if (!ULMai->hwBarrier.pool[poolIndex][ele].inUse) {
                        ...
                        ULMai->hwBarrier.pool[poolIndex][ele].contextID = contextID;
                        ULMai->hwBarrier.pool[poolIndex][ele].nAllocated = 1;
                        ULMai->hwBarrier.pool[poolIndex][ele].nFreed = 0;
                        ULMai->hwBarrier.pool[poolIndex][ele].inUse = true;
                        // initialize barrier data
                        FETCHOP_STORE_OP(ULMai->hwBarrier.pool[poolIndex][ele].barrierData->
                                         fetchOpVar, FETCHOP_STORE, 0);
                        ULMai->hwBarrier.pool[poolIndex][ele].barrierData->hubIndex = poolIndex;
                        ULMai->hwBarrier.pool[poolIndex][ele].barrierData->commSize =
                            localGroup->groupHostData[localGroup->hostIndexInGroup].
                            nGroupProcIDOnHost;
                        ULMai->hwBarrier.pool[poolIndex][ele].barrierData->releaseCnt = 0;
                        // set barrier type
                        SMPBarrierType = HWSPECIFICBARRIER;
                        // set pointer to barrier data structure
                        barrierData = (void *) ULMai->hwBarrier.pool[poolIndex][ele].barrierData;
                        // set pointer to barrier control structure
                        barrierControl = (void *) (&(ULMai->hwBarrier.pool[poolIndex][ele]));
    ...
    if (!firstInstanceOfContext) {
        ...
        // find resource
        for (int poolIndex = 0; poolIndex < ULMai->hwBarrier.nPools; poolIndex++) {
...
{
    // increment nFreed
    barrierFetchOpDataCtlData *barCtl = (barrierFetchOpDataCtlData *) barrierControl;
    barCtl->nFreed++;
