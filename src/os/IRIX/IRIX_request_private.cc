...
void request::MD_add_contraints_from_file(FILE *f)
{
    char rstr[256];
    char sstr[256];
    int num;
    resource_type rt;
    constraint_info ci;
    constraint_type cmask;

    if (!(f)) {
	ulm_exit((-1, "Aborting \n"));
    }
    //
    // WARNING: THIS IS SO UN-ROBUST IT'S EMBARASSING..
    //
    while(fscanf(f, "%s %s %d", rstr, sstr, &num) != EOF) {
	if (!(strlen(rstr)<256)) {
	    ulm_exit((-1, "Aborting \n"));
	}
	if (!(strlen(sstr)<256)) {
	...
	case 6:  // memory
	    ...
	    break;
	default:
	    cerr << "resource '" << rstr << "' not recognized..  ignoring.." << endl;
	    continue;
	}
	switch(strlen(sstr)) {
	case 7:  // specify the number
	    cmask = C_NO_CONSTRAINTS;
	    break;
	case 8:
	    if (strcmp(sstr, "must_use") == 0) {
		// must use this resource
		cmask = C_MUST_USE;
	    } else {
		// do NOT co-locate a HIPPI device with this resource
		cmask = C_NO_HIPPI;
	    }
	    break;
	case 12:
	    if (strcmp(sstr, "must_not_use") == 0) {
		// must NOT use this resource
		cmask = C_MUST_NOT_USE;
	    } else {
		// use only 1 cpu per node
		if (rt != R_CPU) {
		    cerr << "constraint '" << sstr << "' not recognized..  ignoring.." << endl;
		    continue;
		}
		cmask = C_CPU_PER_NODE;
	    }
	    break;
	case 6:  // do NOT co-locate a CPU with this resource
	    cmask = C_NO_CPU;
	    break;
	case 9:  // do NOT co-locate a MEMORY with this resource
	    cmask = C_NO_MEMORY;
	    break;
	default:
	    cerr << "constraint '" << sstr << "' not recognized..  ignoring.." << endl;
	...
	if (cmask == C_NO_CONSTRAINTS) {
	    // request resource
	    add_constraint(rt, num);
	}
	else {
	    ci.set_type(rt);
	    ci.set_num(num);
	    ci.reset_mask(cmask);
	    add_constraint(rt, 1, &ci);
	}
    }
}
