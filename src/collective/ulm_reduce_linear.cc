...
/*!
 * ulm_reduce - reduce function, linear algorithm
 *
 ...
 * Algorithm
 *
 * Simple ring based approach.  This slow algorithm is used when the
 * function is not commutative.  Operation order agrees with MPI spec:
 * start with proc 0
 */
extern "C" int ulm_reduce_linear(const void *s_buf,
                                 void *r_buf,
                                 int count,
                                 ULMType_t *type,
                                 ULMOp_t *op,
                                 ...
                                 int comm)
{
    ...
    if (s_buf != MPI_IN_PLACE) {
        type_copy(r_buf, s_buf, (size_t) count, type);
    }
    ...
    if (self > 0) {
        rc = ulm_irecv(buf, count, type, self - 1, tag, comm, &req);
        if (rc != ULM_SUCCESS) {
            goto EXIT;
        }
        rc = ulm_wait(&req, &status);
        if (rc != ULM_SUCCESS) {
            goto EXIT;
        }
    ...
    if (self < nproc - 1) {
        rc = ulm_isend(r_buf, count, type, self + 1,
    ...
    /*
     * Result at nproc - 1. Send the result to the root
     */

    if (root != nproc - 1) {
        if (self == nproc - 1) {
            rc = ulm_isend(r_buf, count, type, root,
                           tag, comm, &req, ULM_SEND_STANDARD);
            if (rc != ULM_SUCCESS) {
                goto EXIT;
            }
        ...
        if (self == root) {
            rc = ulm_irecv(buf, count, type, nproc - 1, tag, comm, &req);
            if (rc != ULM_SUCCESS) {
                goto EXIT;
            }
            rc = ulm_wait(&req, &status);
