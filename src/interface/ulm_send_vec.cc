...
{
    ULMRequest_t request[MAX_SENDS_TO_POST];
    ...
    while (numDone < numToSend) {
        ...
        if ((numIncomplete < MAX_SENDS_TO_POST) && (numPosted < numToSend)) {
            ...
            errorCode =
                ulm_isend(buf[numPosted], size[numPosted],
                          dtype[numPosted], destinationProc[numPosted],
