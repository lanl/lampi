...
/*!
 * default - bind a multicast send message descriptor to a path object
 *
 ...
 */
extern "C" int ulm_bind_multicast_message(void *multicastMessage, void *hostMessage)
{
    int errorCode = ULM_SUCCESS, pathCount;
    // UtsendDesc_t *msendDesc = (UtsendDesc_t *)multicastMessage;
    BaseSendDesc_t *sendDesc = (BaseSendDesc_t *)hostMessage;
    BasePath_t *pathArray[MAX_PATHS];

    int globalDestID = communicators[sendDesc->ctx_m]->remoteGroup->mapGroupProcIDToGlobalProcID[sendDesc->dstProcID_m];
	...
	if (pathCount == 1) {
		...
		if (pathArray[i]->getInfo(PATH_TYPE, 0, &ptype, sizeof(pathType), &errorCode)) {
	...
	ulm_err(("ulm_bind_multicast_message error: unable to find path to global process ID %d\n", globalDestID));
    }
