...
extern "C" ULMBufferRange_t *ulm_bsend_alloc(ssize_t size, int freeAtZero)
{
    ...
    ULMBufferRange_t *result = (ULMBufferRange_t *)ulm_malloc(sizeof(ULMBufferRange_t));
    ...
    for (next = prev = lampiState.bsendData->allocations; next != NULL; prev = next, next = next->next) {
    ...
    if (!allocated) {
        ...
        else if ((lampiState.bsendData->bufferLength - prev->offset - prev->len) >= size) {
            result->next = NULL;
            result->offset = prev->offset + prev->len;
            prev->next = result;
            allocated = 1;
...
extern "C" ULMBufferRange_t *ulm_bsend_find_alloc(ssize_t offset, ULMRequestHandle_t request)
{
    ULMBufferRange_t *result = NULL;

    for (result = lampiState.bsendData->allocations; result != NULL; result = result->next) {
        if ( offset >= 0) {
            if ( result->offset == offset ) {
                if ( (result->request == request) || ((result->request == NULL) && 
                    (result->refCount == -1)) ) {
                    break;
                }
            }
        ...
        else if ((result->request == request) && (result->freeAtZeroRefCount != 1)) {
            break;
        }
    }
...
extern "C" int ulm_bsend_increment_refcount(ULMRequestHandle_t request, ssize_t offset)
{
...
extern "C" int ulm_bsend_decrement_refcount(ULMRequestHandle_t request, ssize_t offset)
{
    ULMBufferRange_t *allocation;
