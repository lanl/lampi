...
//
//! form the new group out of the ranges of ranks listed
//
extern "C" int ulm_group_range_incl(int group, int nTriplets, int ranges[][3],
				    int *newGroupIndex)
{
    ...
    //
    int *elementsInList =
        (int *) ulm_malloc(sizeof(int) * grpPtr->groupSize);
    if (!elementsInList) {
    ...
    for (int proc = 0; proc < grpPtr->groupSize; proc++) {
        if (elementsInList[proc] >= 0) {
            procIDs[elementsInList[proc]] = proc;
        }
    }

    retValue = ulm_group_incl(group, nRanks, procIDs, newGroupIndex);
