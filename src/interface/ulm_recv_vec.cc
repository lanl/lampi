...
{
    ULMRequestHandle_t request[MAX_RECVS_TO_POST];
    ...
    while( numDone < numToReceive ) {
        ...
        if( (numIncomplete < MAX_RECVS_TO_POST ) &&
            ...
            errorCode=ulm_irecv(buf[numPosted], size[numPosted], dtype[numPosted],
                                sourceProc[numPosted], tag[numPosted], comm[numPosted],
                                request+freeRequest);
