                ...
                ADIO_Offset offset, ADIO_Request *request, int *error_code)  
{
    ...
    static char myname[] = "ADIOI_PANFS_IWRITECONTIG";
#endif
#endif
    ...
    *request = ADIOI_Malloc_request();
    ...
    ADIOI_PANFS_WriteContig(fd, buf, len, MPI_BYTE, file_ptr_type, offset, 
			  &status, error_code);  
    (*request)->queued = 0;
    ...
    err = ADIOI_PANFS_aio(fd, buf, len, offset, 1, &((*request)->handle));
...
/* call the blocking version. It is faster because it does data sieving. */
    ADIOI_PANFS_WriteStrided(fd, buf, count, datatype, file_ptr_type, 
       ...
       /* the man pages say EPROCLIM, but in reality errno is set to EAGAIN! */
		...
		if (errno == EAGAIN) {
		    ...
		    if (wr) err = aiowrite(fd_sys, buf, len, offset, SEEK_SET, result); 
                    ...
                    FPRINTF(stderr, "Unknown errno %d in ADIOI_PANFS_aio\n", errno);
