...
void ADIOI_PANFS_Fcntl(ADIO_File fd, int flag, ADIO_Fcntl_t *fcntl_struct, int *error_code)
{
    ...
    static char myname[] = "ADIOI_PANFS_FCNTL";
#endif
...
#ifdef PRINT_ERR_MSG
		FPRINTF(stderr, "ADIOI_PANFS_Fcntl: To preallocate disk space, ROMIO needs to read the file and write it back, but is unable to read the file. Please give the file read permission and open it with MPI_MODE_RDWR.\n");
	...
	*error_code = MPI_SUCCESS;
    ...
    default:
	FPRINTF(stderr, "Unknown flag passed to ADIOI_PANFS_Fcntl\n");
	MPI_Abort(MPI_COMM_WORLD, 1);
    }
}
